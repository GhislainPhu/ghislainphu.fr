/* https://github.com/bramstein/fontfaceobserver/releases/tag/v1.7.1 */
...
( function ( f ) {
  var fontLoaded = function ( font ) {
    var className = [ font.family, font.weight, font.style ].join( '-' ).toLowerCase();
  ...
  Object.keys( fonts ).forEach( function ( font ) {
    fonts[ font ].forEach( function ( variant ) {
      ( new f( font, variant ) ).load( null, 15000 ).then( fontLoaded );
    });
  });
})( FontFaceObserver );
